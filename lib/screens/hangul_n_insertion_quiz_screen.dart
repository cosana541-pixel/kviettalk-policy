...
class HangulNInsertionQuizScreen extends StatefulWidget {
  ...
  @override
  State<HangulNInsertionQuizScreen> createState() =>
      _HangulNInsertionQuizScreenState();
}

class _HangulNInsertionQuizScreenState
    extends State<HangulNInsertionQuizScreen> {
  ...
  Widget _buildResult(BuildContext context) {
    ...
    return ListView(
      key: const Key('n-insertion-quiz-result'),
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          '$_score/${hangulNInsertionQuizQuestions.length} 정답 · câu đúng',
          key: const Key('n-insertion-quiz-score'),
          textAlign: TextAlign.center,
        ...
        FilledButton.icon(
          key: const Key('retry-n-insertion-quiz'),
        ...
        OutlinedButton.icon(
          key: const Key('back-to-n-insertion-learning'),
          style: OutlinedButton.styleFrom(
            minimumSize: const Size.fromHeight(50),
          ),
          onPressed: () => Navigator.of(context).pop(),
          icon: const Icon(Icons.menu_book),
          label: const Text('ㄴ 첨가 학습으로 · Quay lại bài học'),
        ),
      ],
    );
  }
  ...
  void _nextQuestion() {
    if (_selectedAnswer == null) return;
    setState(() {
      if (_questionIndex == hangulNInsertionQuizQuestions.length - 1) {
        _isComplete = true;
      } else {
        _questionIndex++;
        _displayedOptions = createShuffledHangulQuizOptions(_question.options);
...
class _NInsertionAnswerCard extends StatelessWidget {
  const _NInsertionAnswerCard({
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      color: color,
      child: InkWell(
        key: ValueKey('n-insertion-option-$option'),
