...
class HangulNasalizationQuizScreen extends StatefulWidget {
  ...
  @override
  State<HangulNasalizationQuizScreen> createState() =>
      _HangulNasalizationQuizScreenState();
}

class _HangulNasalizationQuizScreenState
    extends State<HangulNasalizationQuizScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('비음화 퀴즈 · Quiz')),
      body: SafeArea(
    ...
    return ListView(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: LinearProgressIndicator(
                value:
                    (_questionIndex + 1) /
                    hangulNasalizationQuizQuestions.length,
              ),
            ),
            const SizedBox(width: 12),
            Text(
              '${_questionIndex + 1}/${hangulNasalizationQuizQuestions.length}',
              style: Theme.of(
                context,
        ...
        if (isAnswered) ...[
              ...
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    key: const Key('nasalization-quiz-feedback'),
          ...
          FilledButton(
            key: const Key('next-nasalization-question'),
            style: FilledButton.styleFrom(
              minimumSize: const Size.fromHeight(50),
            ),
            onPressed: _nextQuestion,
            child: Text(
              _questionIndex == hangulNasalizationQuizQuestions.length - 1
                  ? '결과 보기 · Xem kết quả'
                  : '다음 문제 · Câu tiếp theo',
            ),
          ),
  ...
  Widget _buildResult(BuildContext context) {
    ...
    return ListView(
      key: const Key('nasalization-quiz-result'),
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          '$_score/${hangulNasalizationQuizQuestions.length} 정답 · câu đúng',
          key: const Key('nasalization-quiz-score'),
          textAlign: TextAlign.center,
        ...
        FilledButton.icon(
          key: const Key('retry-nasalization-quiz'),
        ...
        OutlinedButton.icon(
          key: const Key('back-to-nasalization-learning'),
          style: OutlinedButton.styleFrom(
            minimumSize: const Size.fromHeight(50),
          ),
          onPressed: () => Navigator.of(context).pop(),
          icon: const Icon(Icons.menu_book),
          label: const Text('비음화 학습으로 · Quay lại bài học'),
        ),
      ],
    );
  }
  ...
  void _nextQuestion() {
    if (_selectedAnswer == null) return;
    setState(() {
      if (_questionIndex == hangulNasalizationQuizQuestions.length - 1) {
        _isComplete = true;
      } else {
        _questionIndex++;
        _displayedOptions = createShuffledHangulQuizOptions(_question.options);
...
class _NasalizationAnswerCard extends StatelessWidget {
  const _NasalizationAnswerCard({
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      color: color,
      child: InkWell(
        key: ValueKey('nasalization-option-$option'),
